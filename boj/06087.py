            ...
            for delta in range(4):
                if graph[crt_r+dx[delta]][crt_c+dy[delta]] != WALL: # 벽이 아니면 전진
                    if crt_direct == delta: #같은 방향이라면 가중치 더하기x
                        if crt_cost <= distance[crt_r+dx[delta]][crt_c+dy[delta]]: # 현재 가중치 + 다음 가중치 < 다음 노드의 distance보다 작으면 큐에 넣기
                            # 일단 후보군에 넣어
                            ...
                            continue;
                    elif crt_direct == -delta: # 반대 방향이면 넣지 않기
                        continue;
                    else: # 좌우라면 가중치 + 1
                        if crt_cost + 1 <= distance[crt_r+dx[delta]][crt_c+dy[delta]]: # 현재 가중치 + 다음 가중치 < 다음 노드의 distance보다 작으면 큐에 넣기
                            # 일단 후보군에 넣어
